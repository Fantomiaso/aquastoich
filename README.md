...
TDS can be entered for source water and each aquarium, used as a target with an optional allowed range, and saved in the measurement log in ppm. The calculated TDS adds the mass of **known ions** from dosed products (mg/L, approximately ppm in freshwater) to the starting meter reading. During a water change, it first mixes the aquarium and source readings by volume. The app does not convert conductivity to TDS, and a meter's ppm scale may differ from this mass estimate. Undisclosed product ingredients, precipitation, and reactions can also change the measured result. An absent starting TDS is shown as unknown and blocks automatic fitting to a TDS target; verify calculated values with the same meter.
