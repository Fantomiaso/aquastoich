...
## [⬇ Download AquaStoich 1.0 · Windows / macOS / Linux](https://github.com/Fantomiaso/aquastoich/releases/tag/v1.0.0)

**Version 1.0 is a testing release.** AquaStoich calculates freshwater aquarium remineralization, fertilizer doses, stock solutions, ion ratios, and water changes. Verify doses and pH with measurements before using the prepared water.

## Download and install

Download the matching package from [release 1.0](https://github.com/Fantomiaso/aquastoich/releases/tag/v1.0.0): Windows installer or portable `.exe`, macOS `.dmg`, Debian/Ubuntu `.deb`, or Linux `.AppImage`. The first launch uses English; the language selector in the top bar offers English, Russian, German, and Spanish. The testing builds are unsigned, so your operating system may ask you to confirm opening them.
...
- Records lighting with each measurement: fixture, power, intensity, start time, duration, color temperature, and PAR. Exports the selected aquarium's log to an Excel `.xlsx` file, including test notes and lighting columns.
...
4. Choose substances in **Substances and doses**. Search by name, synonym, or formula and filter by effect. Effects use `Mg+`, `KH−`, etc. Sort by name, effect, or substance/mixture/solution.
...
8. In **Measurement log**, record readings and optional lighting settings for the selected aquarium. Notes are available for the whole entry and each test. Use **Export to Excel** to save that aquarium's log.
